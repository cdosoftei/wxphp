...
[  --with-wxwidgets[=DIR]    Enable wxWidgets extension (requires wxWidgets >= 3.2.2.1).])
...
if test "$PHP_WXWIDGETS" != "no"; then
    ...
    if test "$PHP_WXWIDGETS" != "yes"; then
        AC_MSG_CHECKING([for wx-config existance and wxWidgets version >= 3.2.x])
        for directory in "$PHP_WXWIDGETS" "$PHP_WXWIDGETS/bin" /usr /usr/bin /usr/local /usr/local/bin; do
            ...
            for cmd in wx-config-3.2 wx-config; do
                if test -e "$directory/$cmd"; then
                    wxwidgets_version=`$directory/$cmd --version`
                    version_check=`echo $wxwidgets_version | grep "3.2" && echo $wxwidgets_version | grep "0.[0-9]"`
